...
Diagnostiquer et restaurer la détection de l’extenseur I²C **CH422G** (plage d’adresses 0x20–0x27, valeur usine 0x24) sur la Waveshare **ESP32-S3 Touch LCD 7B** afin de récupérer la ligne **CS microSD**, les sorties EXIO (rétroéclairage, GPIO externes) et d’éviter les boucles de redémarrage liées aux échecs de montage SD.
...
4. **Pont de secours CS microSD**
   ...
   - Activer `CONFIG_SD_USE_FALLBACK_GPIO_CS` **uniquement** une fois le pont en place et définir `CONFIG_SD_FALLBACK_CS_GPIO=34` pour utiliser la liaison directe.
...
| `Component config → CH422G expander` | `I2C address 0x24` (plage automatique 0x20–0x27), `EXIO SD-CS=4` | Assure la correspondance avec le routing Waveshare (EXIO4 → microSD-CS). |
| `Component config → Storage / SD card → Bypass CH422G and drive SD card CS from an ESP32-S3 GPIO` | **Désactivé** par défaut | À activer uniquement si la CS est recâblée directement vers le MCU. |
| `Component config → Storage / SD card → Fallback GPIO number controlling SD card CS` | `34` | GPIO recommandé pour un câblage direct sur la Waveshare 7B. |
| `Component config → Storage / SD card → Automatically mount the SD card at boot` | **Activé** | Monte le volume au reset pour exposer `/sdcard`. |
| `Component config → Storage / SD card → Mount point for the SD card` | `/sdcard` | Adapter selon les besoins applicatifs. |

Pour bypasser définitivement l’extenseur, activer `CONFIG_SD_USE_FALLBACK_GPIO_CS` et fixer `CONFIG_SD_FALLBACK_CS_GPIO=34`. Cette option exige un câblage permanent de la CS vers le MCU.

## Analyse des journaux `idf.py monitor`

Les messages suivants valident l’état du système :

- `CH422G ready on 0x24` (ou 0x20–0x27 selon straps) : l’extenseur répond et la CS repasse par EXIO4.
...
- `I2C scan (0x08-0x77): …` : instantané automatique des périphériques répondant sur le bus (GT911, sondes). Il doit inclure
  `0x20–0x27` lorsque le CH422G est alimenté.

En cas d’erreur :

| Message | Cause probable | Action |
|---------|----------------|--------|
| `No ACK from CH422G between 0x20 and 0x27` | Absence d’alimentation, SDA/SCL coupés, CH422G HS. | Reprendre la section **Étapes matérielles**. |
...
| `CH422G responded on 0x21 instead of configured 0x24` | Strap A0/A1 différent ou module de remplacement. | Mettre `CONFIG_CH422G_I2C_ADDR=0x21`. |
...
| `Le contrôleur tactile GT911 reste visible …` | GT911 (0x5D/0x14) actif mais pas de 0x20–0x27 → panne localisée sur le CH422G ou sa nappe LCD. | Inspecter le pad `LCD_VDD_FR`, la nappe 40 broches et l’extenseur. |
...
3. `Aucun périphérique n'a répondu entre 0x20 et 0x27.` — revenir aux tests matériels (3V3, nappes, pont de secours).
