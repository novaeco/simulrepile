...
void ui_root_init(lv_disp_t *disp)
{
    ...
    for (size_t i = 0; i < APP_MAX_TERRARIUMS; ++i) {
        ui_root_update_terrarium(i);
    }
    ui_root_show_dashboard(0);
}

void ui_root_show_dashboard(size_t terrarium_index)
{
    lv_obj_add_flag(ui_docs_container(), LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(ui_settings_container(), LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_flag(ui_dashboard_container(), LV_OBJ_FLAG_HIDDEN);
    ui_dashboard_show(terrarium_index);
}

void ui_root_show_documents(void)
{
    lv_obj_add_flag(ui_dashboard_container(), LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(ui_settings_container(), LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_flag(ui_docs_container(), LV_OBJ_FLAG_HIDDEN);
    ui_docs_show();
}
...
void ui_root_show_settings(void)
{
    lv_obj_add_flag(ui_dashboard_container(), LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(ui_docs_container(), LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_flag(ui_settings_container(), LV_OBJ_FLAG_HIDDEN);
    ui_settings_show();
}
